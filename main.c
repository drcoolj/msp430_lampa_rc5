...
/* todo !!! debug poprawic odbior komendy (zidentyfikowac przyczyne, port config pull up?)
...
*/
				...
				case eKomenda:
//					if ((rcvPortOst != 0) && !(rcvPortCur)) {
//					if (!(rcvPortCur) && (rcvLicznik>2) && !(P1IN & IR_DATA)) {
					if (!(rcvPortCur)) {
...
//						else if ((rcvLicznik >= dlugosc_pauza-dlugosc_korekcja) && (rcvLicznik <= dlugosc_pauza+dlugosc_korekcja))
						else
						{ // pauza
							// powtorka++;
						}
...
//				rcvPortOst = rcvPortCur;	// zapamietanie aktualnego stanu portu
